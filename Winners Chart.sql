...
FROM (
  ...
  DENSE_RANK() OVER (PARTITION BY event_id ORDER BY MAX(score) DESC) AS ranking FROM scoretable
  GROUP BY event_id, participant_name
) AS rankings
WHERE ranking <= 3 GROUP BY event_id ORDER BY event_id ASC;
